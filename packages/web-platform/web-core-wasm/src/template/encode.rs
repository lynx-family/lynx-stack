...
use super::{decode_legacy_json::JsonTemplateRaw, LynxTemplate};
#[cfg(feature = "encode")]
...
pub fn encode_template(json_template: &wasm_bindgen::JsValue) -> js_sys::Uint8Array {
  let json_template =
    serde_wasm_bindgen::from_value::<JsonTemplateRaw>(json_template.clone()).unwrap();
  let lynx_template: LynxTemplate = json_template.into();
