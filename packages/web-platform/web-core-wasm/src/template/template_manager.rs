...
impl TemplateManager {
  ...
  pub fn set_config(
    ...
    let (config, _) = bincode::serde::decode_from_slice::<Configurations, _>(
      &config_buf.to_vec(),
      bincode::config::standard(),
    )
    .map_err(|e| {
      JsError::new(&format!(
        "Failed to decode Configurations from Uint8Array: {e}"
      ))
    })?;
    self
      .cache
  ...
  pub fn set_style_info(
    ...
    let (raw_style_info, _) = bincode::serde::decode_from_slice::<RawStyleInfo, _>(
      &style_info_buf.to_vec(),
      bincode::config::standard(),
    )
    .map_err(|e| {
      JsError::new(&format!(
        "Failed to decode RawStyleInfo from Uint8Array: {e}"
      ))
    })?;
  ...
  pub fn set_lepus_code(
    ...
    let (code_section, _) = bincode::serde::decode_from_slice::<CodeSection, _>(
      &code_section_buf.to_vec(),
      bincode::config::standard(),
    )
    .map_err(|e| {
      JsError::new(&format!(
        "Failed to decode CodeSection from Uint8Array: {e}"
      ))
    })?;
    let lepus_code = LepusCode::new(code_section, is_lazy_component_template, &template_url);
  ...
  pub fn set_background_code(
    ...
    let template = self
      ...
      .get_mut(&template_url)
      .ok_or_else(|| JsError::new("Template not found"))?;
    template.background_code_urls = Some(decode_code_section_for_background(code_section_buf));
    Ok(())
  }
  ...
  pub fn set_element_template_section(
    ...
    let template = self
      ...
      .get_mut(&template_url)
      .ok_or_else(|| JsError::new("Template not found"))?;
    let (element_template_section, _) =
      bincode::serde::decode_from_slice::<ElementTemplateSection, _>(
        &element_template_section_buf.to_vec(),
        bincode::config::standard(),
      )
      .map_err(|e| {
        JsError::new(&format!(
