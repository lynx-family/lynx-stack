...
pub(crate) struct DecodedStyleInfo {
  ...
  /**
   * Indicates whether to generate declaration block wrapper `{}`.
   * If `enableCSSSelector` is true, we always generate the declaration block.
   * If `enableCSSSelector` is false, we only generate the declaration block
   * when the current rule has selectors other than single class selector.
   *
   */
  generate_declaration_block: bool,
...
impl DecodedStyleInfo {
        ...
        match style_rule.rule_type {
          RuleType::Declaration => {
            if !self.config_enable_css_selector {
              self.generate_declaration_block = false; // it will be set to true if there exists any non-single-class-selector
              ...
              */
              ...
              if !self.config_enable_css_selector {
                    ...
                    == OneSimpleSelectorType::ClassSelector
                {
                  ...
                  self.generate_declaration_block = true;
                }
              }
  ...
  fn generate_one_declaration_block(&mut self, declaration_block: DeclarationBlock) {
    ...
    for declaration in declaration_block.declarations.into_iter() {
      // Feed property name
      ...
      for token in declaration.value_token_list.into_iter() {
...
impl Generator for DecodedStyleInfo {
  fn push_transform_kids_style(&mut self, declaration: ParsedDeclaration) {
    if self.generate_declaration_block {
      ...
      for class_selector_name in self
        ...
        .into_iter()
      {
        let strint_buf = class_selector_map.entry(class_selector_name).or_default();
...
mod test {
  ...
  fn generate_string_buf(raw_style_info: RawStyleInfo) -> DecodedStyleInfo {
    DecodedStyleInfo::new(raw_style_info, None, true)
  }
