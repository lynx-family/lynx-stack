...
impl MainThreadWasmContext {
  ...
  pub fn element_from_binary(
      ...
      .and_then(|map| map.get(&element_template_name))
    {
      ...
      let element_template_section = template_manager.get_element_template_by_name(&template_url);
      if let Some(element_template_section) = element_template_section {
        ...
        let mut id_to_prepared_element_data: FnvHashMap<i32, LynxElementData> =
          FnvHashMap::default();
          ...
          match operation.opcode {
            LEOAsmOpcode::CreateElement => {
              ...
              let dom = self
                ...
                .map_err(|e| {
                  JsError::new(&format!("Failed to create element {tag_name}: {e:?}"))
                })?;
        ...
        Err(wasm_bindgen::JsError::new("Element template not found"))
      }
    }
  }
}
