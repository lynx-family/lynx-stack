...
describe('Bundle Build', () => {
  ...
  test('napi-modules bundle', async () => {
    const rsbuild = await createRsbuild({
      rsbuildConfig: {
        source: {
          entry: {
        ...
        output: {
          distPath: {
            root: path.resolve(__dirname, './dist/napi-modules-bundle'),
          },
        },
        plugins: [
          pluginWebPlatform({
            nativeModulesPath: path.resolve(
              __dirname,
              './fixtures/index.napi-modules.ts',
            ),
          }),
        ],
        performance: {
    ...
    for (
      ...
      for (const m of modules) {
        if (
          m.type === 'javascript/auto'
          && m.userRequest.includes('tests/fixtures/index.napi-modules.ts')
        ) {
          if (!i.isOnlyInitial()) {
            asyncChunkImportCount++;
          } else {
