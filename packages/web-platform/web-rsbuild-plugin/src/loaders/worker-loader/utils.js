...
function workerGenerator(loaderContext, workerFilename, workerSource, options) {
  ...
  if (options.inline) {
    const InlineWorkerPath = stringifyRequest(
      loaderContext,
      `!!${fileURLToPath(new URL('./runtime/inline.js', import.meta.url))}`,
    );

    let fallbackWorkerPath;

    if (options.inline === 'fallback') {
      fallbackWorkerPath = `__webpack_public_path__ + ${
        JSON.stringify(
          workerFilename,
        )
      }`;
...
${
    ...
    }, ${JSON.stringify(workerConstructor)}, ${
      JSON.stringify(
        workerOptions,
      )
    }, ${fallbackWorkerPath});\n}\n`;
  ...
  } function ${fnName}() {\n  return new ${workerConstructor}(__webpack_public_path__ + ${
    JSON.stringify(
      workerFilename,
    )
