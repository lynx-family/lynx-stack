...
const RENAME_RULE: [Option<TrieNodeLeaf>; get_rule_size(&RENAME_RULES_RAW)] =
...
const REPLACE_RULE: [Option<TrieNode>; REPLACE_RULES_RAW_SIZE] = {
...
fn get_trie_leaf_value(
  ...
  let mut ii = start;
  let len = core::cmp::min(end, source.len());
  while ii < len {
    let raw_code = source[ii];
    let code = get_trie_char_code!(raw_code) as usize;
    if leaves[ii].is_some() {
      let trie_node_leaf = leaves[ii].as_ref().unwrap();
...
fn get_trie_node_value(
  ...
  while ii < len {
    let raw_code = source[ii];
    let code = get_trie_char_code!(raw_code) as usize;
    if trie_nodes[ii].is_some() {
      let trie_node = trie_nodes[ii].as_ref().unwrap();
...
pub fn get_replacement(
  ...
  // check the replace rules at first
  if let Some(leaves) = get_trie_node_value(source, name_start, name_end, &REPLACE_RULE) {
    if let Some(value) = get_trie_leaf_value(source, value_start, value_end, &leaves) {
      return Some(value);
    }
  }
  // check the rename rules
  if let Some(value) = get_trie_leaf_value(source, name_start, name_end, &RENAME_RULE) {
    return Some(value);
  }
  return const { None };
}
...
mod tests {
  ...
  fn test_rename_rule_flex_direction() {
    ...
    let value_start = source.len() - 3;
    let value_end = source.len();
    let result = get_replacement(&source, name_start, name_end, value_start, value_end);
    assert_eq!(result, Some("--flex-direction:row"));
  }
}
