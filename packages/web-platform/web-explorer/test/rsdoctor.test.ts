...
vi.mock('@rsbuild/core', () => ({ defineConfig: mocks.defineConfig }));
...
async function loadConfig(rsdoctor?: string) {
  ...
  if (rsdoctor === undefined) delete process.env.RSDOCTOR;
  else process.env.RSDOCTOR = rsdoctor;

  return (await import('../rsbuild.config.js')).default;
}

afterEach(() => {
  if (originalRsdoctor === undefined) delete process.env.RSDOCTOR;
  else process.env.RSDOCTOR = originalRsdoctor;
});
