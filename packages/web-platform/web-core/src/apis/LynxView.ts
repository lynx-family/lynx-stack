 ...
 * @property {"all-on-ui" | "multi-thread"} threadStrategy [optional] (attribute: "thread-strategy") controls the thread strategy for current lynx view
      ...
      queueMicrotask(() => {
        ...
        if (this.#url) {
          ...
          const threadStrategy =
            (this.getAttribute('thread-strategy') ?? 'multi-thread') as
              | 'all-on-ui'
              | 'multi-thread';
