...
lazy_static::lazy_static! {
  ...
  pub static ref RENAME_RULE: HashMap<&'static str, &'static str> = {
    let mut map = HashMap::new();
    map.insert("linear-weight", "--lynx-linear-weight");
    map.insert("flex-direction", "--flex-direction");
    map.insert("flex-wrap", "--flex-wrap");
    map.insert("flex-grow", "--flex-grow");
    map.insert("flex-shrink", "--flex-shrink");
    map.insert("flex-basis", "--flex-basis");
    map.insert("list-main-axis-gap", "--list-main-axis-gap");
    map.insert("list-cross-axis-gap", "--list-cross-axis-gap");
    map
  };

  pub static ref REPLACE_RULE: HashMap<&'static str, HashMap<&'static str, &'static [[&'static str; 2]]>> = {
    let mut map = HashMap::new();

    // Display rules
    let mut display_map = HashMap::new();
    display_map.insert("linear", &[
...
macro_rules! get_rename_rule_value {
  ($name_slice:expr, $start:expr, $end:expr) => {
    crate::transformer::rules::RENAME_RULE
      .get(&*String::from(
        $name_slice.slice($start as u32, $end as u32),
      ))
      .copied()
  };
}
...
macro_rules! get_replace_rule_value {
  ($name_slice:expr, $name_start:expr, $name_end:expr, $value_slice:expr, $value_start:expr, $value_end:expr) => {
    if let Some(sub_rule) = crate::transformer::rules::REPLACE_RULE.get(&*String::from(
      $name_slice.slice($name_start as u32, $name_end as u32),
    )) {
      sub_rule
        .get(&*String::from(
          $value_slice.slice($value_start as u32, $value_end as u32),
        ))
        .copied()
    } else {
      None
