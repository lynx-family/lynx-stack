...
mod tests {
    ...
    fn test_multiple_declarations() {
        ...
        assert_eq!(transformer.get_name(&source, &transformer.declarations[0]), "color");
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "red");
        assert!(!transformer.declarations[0].is_important);

        assert_eq!(transformer.get_name(&source, &transformer.declarations[1]), "margin");
        assert_eq!(transformer.get_value(&source, &transformer.declarations[1]), "10px");
        assert!(!transformer.declarations[1].is_important);

        assert_eq!(transformer.get_name(&source, &transformer.declarations[2]), "padding");
        assert_eq!(transformer.get_value(&source, &transformer.declarations[2]), "5px");
    ...
    fn test_complex_values() {
        ...
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_name(&source, &transformer.declarations[0]), "background");
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "url(image.png) no-repeat center");
    }
    ...
    fn test_hyphenated_properties() {
        ...
        assert_eq!(transformer.declarations.len(), 2);
        assert_eq!(transformer.get_name(&source, &transformer.declarations[0]), "font-size");
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "14px");
        assert_eq!(transformer.get_name(&source, &transformer.declarations[1]), "background-color");
        assert_eq!(transformer.get_value(&source, &transformer.declarations[1]), "blue");
    }
    ...
    fn test_parser_edge_cases() {
        ...
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "url(test.png)");
    }
    ...
    fn test_important_edge_cases() {
        ...
        let (transformer, source) = parse_css("color: red ! important ;");
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "red ! important");
        ...
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "red !important extra");
        assert!(transformer.declarations[0].is_important);
    }
    ...
    fn test_special_characters_and_escapes() {
        ...
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "#fff");
    ...
    fn test_numeric_values() {
        ...
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "10");

        // Test decimal values
        let (transformer, source) = parse_css("opacity: 0.5;");
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "0.5");

        // Test negative values
        let (transformer, source) = parse_css("margin: -10px;");
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "-10px");

        // Test percentage values
        let (transformer, source) = parse_css("width: 100%;");
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "100%");
    }
    ...
    fn test_string_values() {
        ...
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "\"hello world\"");
    ...
    fn test_function_values() {
        ...
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "calc(100% - 20px)");

        // Test rgb function
        let (transformer, source) = parse_css("color: rgb(255, 0, 0);");
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "rgb(255, 0, 0)");

        // Test nested functions
        let (transformer, source) = parse_css("transform: translateX(calc(100% + 10px));");
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "translateX(calc(100% + 10px))");
    }
    ...
    fn test_comments() {
        ...
        assert_eq!(transformer.declarations.len(), 1);
        assert_eq!(transformer.get_value(&source, &transformer.declarations[0]), "red /* comment */");
    ...
    fn test_string_tokenizer_edge_cases() {
        ...
        assert!(collector.tokens.iter().any(|(token_type, _, _)| *token_type == BAD_STRING_TOKEN));
    ...
    fn test_url_tokenizer_edge_cases() {
        ...
        let mut collector = TokenCollector::new();
        tokenize(&source, &mut collector);
        assert!(collector.tokens.iter().any(|(token_type, _, _)| *token_type == FUNCTION_TOKEN));
    ...
    fn test_numeric_tokenizer_edge_cases() {
        ...
        let mut collector = TokenCollector::new();
        tokenize(&source, &mut collector);
        assert!(collector.tokens.iter().any(|(token_type, _, _)| *token_type == DIMENSION_TOKEN));

        // Test percentage token
        let source: Vec<u16> = "123%".encode_utf16().collect();
        let mut collector = TokenCollector::new();
        tokenize(&source, &mut collector);
        assert!(collector.tokens.iter().any(|(token_type, _, _)| *token_type == PERCENTAGE_TOKEN));

        // Test number with decimal
        let source: Vec<u16> = "123.456".encode_utf16().collect();
        let mut collector = TokenCollector::new();
        tokenize(&source, &mut collector);
        assert!(collector.tokens.iter().any(|(token_type, _, _)| *token_type == NUMBER_TOKEN));
    ...
    fn test_comment_tokenizer() {
        ...
        let mut collector = TokenCollector::new();
        tokenize(&source, &mut collector);
        assert!(collector.tokens.iter().any(|(token_type, _, _)| *token_type == COMMENT_TOKEN));
        ...
        let mut collector = TokenCollector::new();
        tokenize(&source, &mut collector);
        assert!(collector.tokens.iter().any(|(token_type, _, _)| *token_type == DELIM_TOKEN));
    }
