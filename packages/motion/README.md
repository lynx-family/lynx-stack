...
`motion.image` are included. Use
`motion.create(Component)` for components that forward `style`, host props, and
`main-thread:ref` to a Lynx element.

Consumer easing functions and `transformTemplate` closures use ordinary Motion
syntax—no main-thread directive is required:
...
<motion.div
  ...
  transformTemplate={({ x }, generated) => `translateY(-10px) ${generated}`}
/>;
```
