...
export type MotionTransition = MainThreadDeep<AnimationOptions>;
...
/**
 * Motion props whose function positions execute on the main thread.
 *
 * This public type is also the source for the package's generated
 * directive-inference manifest.
 */
export interface MotionMainThreadProps {
  /** Options used when animating to `animate`. */
  transition?: MotionTransition;
  /**
   * Composes custom transform text around Motion's generated transform.
   * The callback executes on the main thread.
   */
  transformTemplate?:
    & TransformTemplate
    & MainThreadFn<
      Parameters<TransformTemplate>,
      ReturnType<TransformTemplate>
    >;
}

/** Motion-specific props shared by all declarative Motion components. */
export interface MotionProps extends MotionMainThreadProps {
...
export interface MotionFactory {
  /** Upstream-compatible alias that renders a Lynx `view`. */
  div: FunctionComponent<MotionViewProps>;
...
export interface DirectiveInferenceManifest {
  motion: {
    div: MainThreadComponent<MotionMainThreadProps>;
    image: MainThreadComponent<MotionMainThreadProps>;
    text: MainThreadComponent<MotionMainThreadProps>;
    view: MainThreadComponent<MotionMainThreadProps>;
  };
}
