...
`@lynx-js/a2ui-catalog-extractor` 是 `a2ui-cli generate catalog` 背后的
内部 TypeDoc extraction engine。它会把 TypeScript 组件接口转换成 A2UI
组件 catalog JSON。你只需要用 TypeScript `interface` 写一次组件的公开契约，
用普通 TypeDoc 注释描述字段，然后通过公开的 `a2ui-cli` 命令生成 A2UI agent
可以读取的 JSON Schema。

用户脚本请使用 `a2ui-cli generate catalog`。这个包主要作为 extraction
行为和测试的实现层。
...
公开 CLI 入口是 `a2ui-cli generate catalog`。它会在内部委托给这个包。
...
这些导出主要面向 `a2ui-cli` 维护者、extractor 测试和仓库内部工具；它们不是给产品代码接入的外部集成面。
产品构建脚本应该调用 `a2ui-cli generate catalog`，而不是直接 import 这个包。
