...
pub struct JudgePageRequest {
  ...
  /// dimension, optional UI-Bench opponent operations and pairwise vote, and
  /// optional reference-image comparison each receive this full duration. This
  /// preserves the legacy UI Judge behavior and is not an overall deadline for
  /// the entire request.
...
impl CapturedEvaluation {
  pub(crate) fn screenshot_data_url(&self) -> String {
    self.candidate.screenshot_data_url()
  }
}
...
pub(crate) async fn capture_prepared_evaluation(
  ...
  capture_prepared_evaluation_with_options(client, request, &PageLoadOptions::default()).await
}

pub(crate) async fn capture_prepared_evaluation_with_options(
  client: &ModelClient,
  request: &JudgePageRequest,
  load_options: &PageLoadOptions,
) -> Result<CapturedEvaluation, UiJudgeResult> {
  let candidate = capture_prepared_page_with_options(client, request, load_options).await?;
  let ui_bench_opponent = match request.ui_bench_opponent_url.as_deref() {
    ...
    Some(opponent_url) => {
      ...
      Some(
        capture_prepared_page_with_options(client, &opponent_request, load_options)
          .await
...
pub(crate) async fn score_captured_evaluation(
  ...
  let (mut result, comparison_result, ui_bench_result) =
    tokio::join!(vlm_scoring, reference_comparison, ui_bench_evaluation);
