...
use crate::headless::{
  capture_prepared_evaluation, prepare_judge_page_request, score_captured_evaluation,
  CapturedEvaluation,
};
...
fn run_headless_worker(runtime: Runtime, receiver: Receiver<CaptureJob>) {
  while let Ok(job) = receiver.recv() {
    ...
    let capture = runtime.block_on(capture_prepared_evaluation(&job.client, &job.request));
...
mod tests {
  ...
  fn request_defaults_match_the_library_contract() {
    ...
    assert!(request.ui_bench_opponent_url.is_none());
    assert!(request.ui_bench_candidate_mu.is_none());
    assert!(request.ui_bench_candidate_sigma.is_none());
    assert!(request.ui_bench_opponent_mu.is_none());
    assert!(request.ui_bench_opponent_sigma.is_none());
  }
