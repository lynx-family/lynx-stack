...
describe('pluginOptimization', () => {
  ...
  test('overrideStrict should not parse png as JavaScript', async () => {
    rstest.stubEnv('NODE_ENV', 'production')

    const root = await mkdtemp(path.join(tmpdir(), 'rsbuild-plugin-png-asset-'))
    try {
      ...
      const rsbuild = await createStubRsbuild({}, root)
