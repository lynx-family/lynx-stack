...
export function resolveLazyBundleFetcher(
  ...
  if (envOverride === 'FetchBundle' && !meets) {
    throw new Error(
      `[ReactWebpackPlugin] REACT_LAZY_BUNDLE_FETCHER=FetchBundle requires `
        + `engineVersion >= ${FETCH_BUNDLE_MIN_ENGINE_VERSION}, but got `
        + `${engineVersion ? `'${engineVersion}'` : '<unset>'}. Older hosts `
        + `do not expose 'lynx.fetchBundle' / 'lynx.loadScript'. Either bump `
        + `'engineVersion' to '${FETCH_BUNDLE_MIN_ENGINE_VERSION}' or higher, `
        + `or unset REACT_LAZY_BUNDLE_FETCHER (the default falls back to `
