...
export function renderMainThreadDefines(
  ...
  // Published from here rather than from the main-thread entry: the entry ships
  // in `@lynx-js/react`, and `__webpack_require__` is a bundler internal that a
  // published runtime must not reach for. This runtime module is generated by
  // the bundler, so it is the right place to know about it.
  const publish = publishRuntime
    ? `  __webpack_require__.${RUNTIME_HANDLE} = ReactLynx;\n`
    : '';
...
/**
 * The property the card's assembled section publishes its runtime members on,
 * for a lazy bundle's section to read.
 */
const RUNTIME_HANDLE = 'mtDefinesRuntime';
  ...
  return class MainThreadDefinesRuntimeModule extends webpack.RuntimeModule {
    ...
    override generate(): string {
      ...
      if (!compilation || !entrypoint) {
        return renderMainThreadDefines([], true);
      }
