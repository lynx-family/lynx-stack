...
class LynxTemplatePluginImpl {
      ...
      compilation.hooks.runtimeRequirementInTree.for(
        ...
        compilation.addRuntimeModule(
          ...
          new LynxAsyncChunksRuntimeModule((asyncChunk) => {
            const filename =
              this.#getLazyBundleNameByChunkId(compilation).get(asyncChunk.id!)
  ...
  #getAsyncChunkGroups(compilation: Compilation) {
    ...
    asyncChunkGroups = groupBy(
      ...
      cg => {
        ...
        // A name changed by the `asyncChunkName` hook is generated by the
        // transform rather than provided by the user. Group such chunk groups
        // by the resolved modules of their dynamic imports so that the same
        // file imported via different paths (relative or alias) produces a
        // single lazy bundle. User-provided `webpackChunkName`s are kept.
  ...
  #getLazyBundleNameByChunkId(compilation: Compilation) {
    ...
    for (
      const [filename, chunkGroups] of Object.entries(
        this.#getAsyncChunkGroups(compilation),
      )
    ) {
