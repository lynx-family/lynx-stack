...
describe('Lynx plugin coverage sanity check', () => {
  for (const [name, plugin] of Object.entries(lynxPlugins)) {
    it(`${name} registers utilities`, () => {
      ...
      const matchUtilities = rs.mocked(api.matchUtilities);
      const addUtilities = rs.mocked(api.addUtilities);
      const addComponents = rs.mocked(api.addComponents);
      const addBase = rs.mocked(api.addBase);
      const matchVariant = rs.mocked(api.matchVariant);
      const matchComponents = rs.mocked(api.matchComponents);
