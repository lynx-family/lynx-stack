...
- Add a repeatable `--header`/`-H` option to the `debug-metadata remap` CLI so extra HTTP headers (e.g. `-H "authorization: Bearer xxx"`) are forwarded when fetching a `debugMetadataUrl` from an auth-gated endpoint. ([#2810](https://github.com/lynx-family/lynx-stack/pull/2810))
