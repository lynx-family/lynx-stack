...
describe('lynx/no-unsupported-properties', () => {
  ...
  it('supports option: disallow (remove from supported list)', async () => {
    const css = '.a { color: red; }';

    const result = await lint(css, [true, { disallow: ['color'] }]);
  ...
  it('supports option precedence: disallow overrides allow', async () => {
    const css = '.a { backdrop-filter: blur(2px); }';

    const result = await lint(css, [
      true,
      { allow: ['backdrop-filter'], disallow: ['backdrop-filter'] },
    ]);
    const texts = getWarningTexts(result).filter((t) =>
      t.includes('Unsupported CSS property')
    );
