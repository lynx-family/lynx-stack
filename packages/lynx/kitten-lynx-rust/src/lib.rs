...
const APP_NAME: &str = "KittenLynxRust";
...
impl Lynx {
  pub async fn connect(options: ConnectOptions) -> Result<Self> {
    install_lynx_core_resource(options.lynx_core_path.as_deref()).await?;
    let env = Env::load()?;
  ...
  pub fn new_page(&self) -> Result<KittenLynxView> {
    ...
    let resources = ResourceContext::new(self.options.resources_path.clone());
  ...
  async fn wait_for_session(
    ...
    while Instant::now() < deadline {
      if let Ok(sessions) = self.runtime.list_sessions().await {
        if std::env::var_os("KITTEN_LYNX_DEBUG").is_some() {
          eprintln!("[kitten-lynx-rust] sessions: {sessions:?}");
        }
...
async fn install_lynx_core_resource(configured_path: Option<&Path>) -> Result<()> {
...
mod tests {
  ...
  #[test]
  fn matches_session_urls_like_kitten_lynx() {
