...
export const circIn: Easing = t => 1 - Math.sin(Math.acos(t));
export const circOut: Easing = t => Math.sin(Math.acos(t - 1));
export const circInOut: Easing = t =>
  t < 0.5
    ? (1 - Math.sqrt(1 - 4 * t * t)) / 2
    : (Math.sqrt(1 - (-2 * t + 2) ** 2) + 1) / 2;
export const backIn: Easing = t => t * t * ((1.70158 + 1) * t - 1.70158);
export const backOut: Easing = t => --t * t * ((1.70158 + 1) * t + 1.70158) + 1;
export const backInOut: Easing = t => {
  const c1 = 1.70158;
  const c2 = c1 * 1.525;
  return t < 0.5
...
export const anticipate: Easing = t => {
  ...
  useMemo(() => {
    function setMotionValue(value: T) {
      ...
      if (!motionValueRef.current) {
        motionValueRef.current = createMotionValue<T>(value);
      }
    }
    if (__BACKGROUND__) {
