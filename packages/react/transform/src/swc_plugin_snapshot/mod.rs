    ...
    if !jsx_is_custom(n) {
      ...
      if jsx_has_dynamic_key(n) && self.parent_element.is_some() {
        ...
        if jsx_is_list(n) {
          self
            .dynamic_parts
            .push(DynamicPart::ListChildren(expr, self.element_index));
        } else {
          self
            .dynamic_parts
            .push(DynamicPart::Children(expr, self.element_index));
        }
  ...
  fn visit_mut_jsx_element(&mut self, node: &mut JSXElement) {
    ...
    let (dynamic_part_attr, dynamic_part_children): (Vec<_>, Vec<_>) = dynamic_part_extractor
      ...
      .partition(|dynamic_part| match dynamic_part {
        ...
        DynamicPart::Children(_, _) | DynamicPart::ListChildren(_, _) => false,
      });
