...
use swc_plugins_shared::jsx_helpers::{
  jsx_children_to_expr, jsx_name, jsx_text_to_str, transform_jsx_attr_str,
};
...
pub(super) fn wrap_in_slot(
  ...
  // ET dynamic children are transported as indexed element slots, not Snapshot
  // wrapper nodes. Keeping the slot marker in JSX form lets the later React
  // lowering decide the final JS shape without losing slot identity.
