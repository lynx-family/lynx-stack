...
pub struct WorkletVisitor {
  ...
  should_inject_runtime_init: bool,
}
...
impl VisitMut for WorkletVisitor {
    ...
    match n {
      ClassMember::Method(_) => {
        ...
        let (worklet_object_expr, register_worklet_stmt) = StmtGen::transform_worklet(
          ...
          &mut self.should_inject_runtime_init,
        );
  ...
  fn visit_mut_module(&mut self, n: &mut Module) {
    ...
    if self.should_inject_runtime_init {
      // Keep the runtime bootstrap in the same main-thread module graph as the
      // generated registrations. Splitting this side-effect import away from the
      // owning asset can make `registerWorkletInternal` run before the runtime
      // globals exist, especially for lazy main-thread chunks.
...
impl WorkletVisitor {
  ...
  pub fn new(mode: TransformMode, cfg: WorkletVisitorConfig) -> Self {
    WorkletVisitor {
      ...
      should_inject_runtime_init: false,
    }
  }
