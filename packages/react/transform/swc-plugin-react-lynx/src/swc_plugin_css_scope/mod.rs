  ...
  C: Comments,
{
  ...
  fn visit_mut_module(&mut self, n: &mut Module) {
    ...
    for import_decl in import_decls {
      ...
      let re = Regex::new(r"\.(scss|sass|css|less|ttss)$").unwrap();
...
mod tests {
  ...
  #[test]
  fn test_css_scope_all() {
    let json = r#""all""#;
    let mode: CSSScope = serde_json::from_str(json).unwrap();
    assert_eq!(mode, CSSScope::All);
  }

  #[test]
  fn test_css_scope_none() {
    let json = r#""none""#;
    let mode: CSSScope = serde_json::from_str(json).unwrap();
    assert_eq!(mode, CSSScope::None);
  }

  #[test]
  fn test_css_scope_modules() {
    let json = r#""modules""#;
    let mode: CSSScope = serde_json::from_str(json).unwrap();
    assert_eq!(mode, CSSScope::Modules);
  }

  #[test]
  fn test_css_scope_unknown() {
    let json = r#""unknown""#;
    let result: Result<CSSScope, _> = serde_json::from_str(json);
    ...
    if let Err(err) = result {
      assert_eq!(
        err.to_string(),
        "value `unknown` does not match any variant of CSSScope"
      );
    }
  }
}
