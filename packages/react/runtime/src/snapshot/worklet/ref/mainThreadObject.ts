...
export function defineMainThreadObjectType<I, O extends object>(
  ...
  // Read accessor-backed fields once so validation and deferred registration
  // operate on the same definition values.
  const { type, create } = definition;
  ...
  if (!isMainThreadLifecycleFunction(create)) {
    throw new Error(
  ...
  // later mutation of `definition` must not change the type or factory that is
  // registered for this token.
  const validatedDefinition = Object.freeze({
    type,
    create,
  });
  ...
  mainThreadObjectTypeDefinitions.set(
    objectType,
    validatedDefinition as unknown as MainThreadObjectTypeDefinition<unknown, object>,
  );
  registerMainThreadObjectDefinition(validatedDefinition);
  return objectType;
}
...
function findNonSerializablePath(
  ...
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index++) {
      ...
      const invalidPath = findNonSerializablePath(
        value[index],
        `${path}.${index}`,
        ancestors,
      );
