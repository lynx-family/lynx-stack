...
export type BackgroundDOM = VNode & BackgroundSnapshotInstance;
...
export function setupDom(vnode: VNode & { type: string }): BackgroundDOM {
  const dom = new BackgroundSnapshotInstance(vnode.type);
  Object.assign(vnode, dom);
  Object.setPrototypeOf(vnode, BackgroundSnapshotInstance.prototype);
  backgroundSnapshotInstanceManager.values.set(dom.__id, vnode as BackgroundSnapshotInstance);
  return vnode as BackgroundDOM;
}
