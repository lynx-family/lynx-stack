...
describe('compact host direct materialization', () => {
  ...
  it('keeps hooks and component/native ordering through compact host ancestry', () => {
    ...
    const host = __etHost('__Card__:_et_compact_root', '_et_compact_root', '__Card__', undefined, undefined, [[
      h(Child, { value: 'a' }),
      h(Child, { value: 'b' }),
    ]], undefined);
  ...
  it('prepares compact host events and MTRefs without changing input attributes', () => {
    ...
    expect(result.rootSubtreeHandles).toEqual([[{ uid: -1, ref: result.rootRefs[0] }]]);
