...
type GlobalWithEnv = typeof globalThis & {
  __OnLifecycleEvent?: ReturnType<typeof rs.fn>;
...
describe('setupLynxEnv', () => {
  ...
  it('installs lepus event bridge and processData handlers', () => {
    ...
    const defaultDataProcessor = rs.fn((data: { value: number }) => ({ doubled: data.value * 2 }));
    const namedDataProcessor = rs.fn((data: { value: number }) => ({ named: data.value }));
