...
import { setupPage, snapshotInstanceManager, traverseSnapshotInstance } from '../../../src/snapshot';
...
describe('reload does not retain the old tree', () => {
  ...
  it('releases the tree an update before first-screen sync replaces', function() {
    // Before the first screen is synced, `updatePage` renders a fresh tree and
    // hydrates it against the old one, the same way a reload does.
  ...
  it('drops every replaced node from the instance manager on reload', function() {
    globalEnvManager.switchToMainThread();
    __root.__jsx = ListMT;
    renderPage({});
  ...
  it('drops every replaced node from the instance manager on an update before first-screen sync', function() {
    globalThis.__FIRST_SCREEN_SYNC_TIMING__ = 'jsReady';
    try {
      globalEnvManager.switchToMainThread();
      __root.__jsx = ListMT;
      ...
      // The manager is a strong reference, so a replaced node left in it is
      // retained outright, cycle or not.
      const stillRegistered = oldNodes.filter(node => snapshotInstanceManager.values.get(node.__id) === node);
      expect(stillRegistered).toHaveLength(0);
    } finally {
      globalThis.__FIRST_SCREEN_SYNC_TIMING__ = 'immediately';
