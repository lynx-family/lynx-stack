...
describe('element ref', () => {
  ...
  it('should work when using error boundary with ref', async function() {
    const ref = vi.fn(() => {
...
describe('ui operations', () => {
  ...
  it('should delay when setState until data be sent to native', async function() {
    const ref1 = vi.fn((ref) => {
      ref.invoke({
        method: 'boundingClientRect',
      }).exec();
    ...
    // set show
    {
      ...
      expect(lynx.getNativeApp().callLepusMethod.mock.calls[0][1].data).toMatchInlineSnapshot(
        `"{"patchList":[{"id":3,"snapshotPatch":[0,"__Card__:__snapshot_a94a8_test_25",3,4,3,[1],1,-2,3,null]}]}"`,
      );
      expect(lynx.createSelectorQuery().constructor.execLog.mock.calls).toMatchInlineSnapshot(`
        [
          [
  ...
  it('should not delay when calling ui operations in events', async function() {
    const ref1 = createRef();
  ...
  it('should delay when calling ui operations in events before hydration', async function() {
    const ref1 = createRef();
  ...
  it('should delay animation ops', async () => {
    let animation;
    const ref1 = vi.fn(() => {
      animation = lynx.getElementById('id1').animate({ animate: 'test' });
      animation.play();
      animation.pause();
      animation.cancel();
      lynx.getElementById('id1').setProperty('property', 'value');
    });
    ...
    // background render
    {
      ...
      expect(lynx.getElementByIdTasks).not.toHaveBeenCalled();
    }
    ...
    // hydrate
    {
      lynxCoreInject.tt.OnLifecycleEvent(...globalThis.__OnLifecycleEvent.mock.calls[0]);
      globalThis.__OnLifecycleEvent.mockClear();
      expect(lynx.getElementByIdTasks.mock.calls).toMatchInlineSnapshot(`
        [
          [
            "animate",
          ],
      ...
      `);
      lynx.getElementByIdTasks.mockClear();
    }
