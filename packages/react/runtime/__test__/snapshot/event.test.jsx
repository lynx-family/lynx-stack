...
describe('eventUpdate', () => {
  it('basic & throw', async function() {
    const handleTap1 = rs.fn().mockImplementation(() => {
      throw new Error('error');
    });
    const handleTap2 = rs.fn();
    function Comp() {
      return (
        <view>
...
describe('event in spread', () => {
  it('add', async function() {
    ...
    spread1 = { bindtap: rs.fn() };
    spread2 = { bindtap: rs.fn() };
    render(<Comp />, __root);
    patch = takeGlobalSnapshotPatch();
    expect(patch).toMatchInlineSnapshot(`[]`);
