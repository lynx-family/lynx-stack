...
// during initialization. Apps use it to pass theme data, feature
// flags, user preferences, or other configuration to the Lynx runtime.
...
globalThis.renderPage = function renderPage() {
  ...
  __AppendElement(desc, __CreateRawText("Global properties from the host application:"));
  ...
  __SetInlineStyles(propsBox, "margin-top:16px; background-color:#f1f5f9; padding:16px; border-radius:8px;");

  const propsText = __CreateText(0);
  __AppendElement(propsBox, propsText);

  if (globalProps && Object.keys(globalProps).length > 0) {
    __AppendElement(propsText, __CreateRawText(JSON.stringify(globalProps, null, 2)));
  } else {
    __AppendElement(propsText, __CreateRawText("(empty — no global props set in the REPL)"));
  }
  ...
  __SetInlineStyles(noteBox, "margin-top:16px; padding:12px; background-color:#fefce8; border-radius:8px;");

  const noteText = __CreateText(0);
  __AppendElement(noteBox, noteText);
  __AppendElement(noteText, __CreateRawText(
    "In a real Lynx app, the host sets globalProps during " +
    "initialization — e.g. theme colors, user locale, or feature flags. " +
    "Access them via lynx.__globalProps or the global __globalProps."
  ));
  __SetInlineStyles(noteText, "font-size:12px; color:#854d0e; line-height:18px;");
