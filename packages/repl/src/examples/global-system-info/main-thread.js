...
globalThis.renderPage = function renderPage() {
  ...
  __AppendElement(desc, __CreateRawText("Device & platform info from the SystemInfo global:"));
  ...
  for (const [key, value] of entries) {
    ...
    __SetInlineStyles(row, "flex-direction:row; padding:8px 12px; background-color:#f8fafc; border-radius:6px; margin-top:8px;");
    ...
    __SetInlineStyles(valueText, "font-size:13px; color:#666; font-family:monospace;");
  }

  // Also accessible via lynx.SystemInfo
  const noteText = __CreateText(0);
  __AppendElement(container, noteText);
  __AppendElement(noteText, __CreateRawText("Also available as lynx.SystemInfo"));
