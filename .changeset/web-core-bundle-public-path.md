...
External bundle builds now encode `isExternalBundle` and `isLazy` in bundle config, so Web Core can prefer producer metadata over FetchBundle caller fallbacks when choosing external-container vs lazy-component decode mode.
