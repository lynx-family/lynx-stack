...
- 类型：`boolean`
- 作用：产物中带有性能埋点时为 `true`。开发构建默认开启，生产构建默认关闭。埋点会产出哪些 trace 见[性能分析](https://lynxjs.org/zh/react/performance/profiling)，如何为生产构建打开见[运行时性能分析](https://lynxjs.org/zh/rspeedy/runtime-profiling)。

## `__REACT_DEVTOOL__`

- 类型：`boolean | undefined`
- 作用：[Preact DevTools](https://lynxjs.org/zh/guide/devtool/panels/preact-devtools-panel) 所依赖的运行时钩子被编译进产物时为 `true`。开发构建默认开启；生产构建可以用环境变量 `REACT_DEVTOOL=true` 打开。

## `__LAZY_BUNDLE_FETCHER__`

- 类型：`'FetchBundle' | 'QueryComponent'`
- 作用：当前产物接入的是哪一种 lazy bundle 拉取方式。`'FetchBundle'` 走基于 `lynx.fetchBundle` 的路径（以及 `import(..., { with: { mode } })` 的 mode 提示），`'QueryComponent'` 是旧的 `lynx.QueryComponent` 路径。取值如何选定、`REACT_LAZY_BUNDLE_FETCHER` 覆盖方式以及版本范围，见[懒加载 bundle 的加载器](https://lynxjs.org/zh/react/code-splitting#懒加载-bundle-的加载器)。
