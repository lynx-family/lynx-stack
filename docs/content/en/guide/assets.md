...
- In development, using [`dev.assetPrefix`](/rspeedy/config/dev#dev-assetprefix) to set the path prefix.
- In production, using [`output.assetPrefix`](/rspeedy/config/output#output-assetprefix) to set the path prefix.
...
The threshold can be modified with the [`output.dataUriLimit`](/rspeedy/config/output#output-dataurilimit) config.
...
If the built-in asset types in Rsbuild cannot meet your requirements, you can modify the built-in Rspack configuration and extend the asset types you need using [`tools.rspack`](/rspeedy/config/tools#tools-rspack).
