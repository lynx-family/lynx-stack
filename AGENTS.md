...
- The HTTP server must honor `LYNX_USE_HOST` when set and otherwise listen on IPv4 and IPv6 unspecified addresses, keep native Lynx capture on its dedicated current-thread worker, bound queued captures, score completed captures concurrently, and drain accepted work during SIGINT or SIGTERM shutdown. Keep CPU-heavy image processing on the bounded Rayon pool so it does not block Tokio workers.
...
- Keep VLM evaluation on Agent SDK and use only the documented `UI_JUDGE_*` model environment variables. Do not accept legacy Midscene- or OpenAI-prefixed variables, JSON init config, or model-family configuration, and do not add Midscene, Playwright, Kitten-Lynx, Android, or ADB dependencies to the crate.
