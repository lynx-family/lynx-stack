...
Do not add `@sparticuz/chromium` or `playwright-core` back to `packages/genui/server`. Keep the Playwright import and browser-backed implementation commented in `a2ui-bench-preview.ts` as the extraction boundary until preview rendering moves to its dedicated service; do not delete that implementation while it is disabled.
