...
Normalize UI Judge's captured BMP data URLs to PNG in the shared GenUI Server judge-response adapter before returning or storing Bench results. Decode only the runner's top-down 32-bit BITMAPV4HEADER layout with the expected RGBA masks, bound input bytes and dimensions before allocating pixels, and preserve alpha during asynchronous PNG compression. Apply the existing per-image PNG limit after conversion and retain the per-job screenshot budget. Invalid or oversized images remain warning-only diagnostics; do not change otherwise valid scores. Keep PNG response compatibility and do not move model scoring or credentials into GenUI Server.
