...
Catalog schema properties use string index signatures. When named property access must satisfy both TypeScript `noPropertyAccessFromIndexSignature` and ESLint `dot-notation`, use destructuring or object helpers such as `Reflect.deleteProperty` and `Object.assign`. Do not switch between bracket and dot access to satisfy only one checker; rerun declaration generation after changing this code.
