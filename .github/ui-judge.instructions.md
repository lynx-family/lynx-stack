...
When generating UI Judge fixture bundles from Vitest, force the fixture build to use `NODE_ENV=production` rather than inheriting Vitest's `NODE_ENV=test`. Rspeedy production aliases mirror the committed example build behavior and avoid resolving dev-only fixture imports such as `@lynx-js/preact-devtools` from non-workspace fixture directories.
