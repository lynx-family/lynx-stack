...
For fixture-backed `@lynx-js/ui-judge` Android E2E tests, do not commit generated `.bundle` artifacts under `packages/genui/ui-judge/tests/fixtures`. Build the React fixture bundle from the Rspeedy React TypeScript starter template when the fixture server starts: copy `packages/rspeedy/create-rspeedy/template-common/src` and `packages/rspeedy/create-rspeedy/template-react-ts/src` into a temporary build root, build that temporary `src/index.tsx`, serve the generated bundle from the test process, and delete it during fixture teardown. Keep the checked-in fixture snapshot image, and reach the served bundle from the emulator with `adb reverse`. Gate the suite with `UI_JUDGE_ANDROID_INTEGRATION` so ordinary Vitest runs do not require an emulator or Midscene credentials. Keep reference snapshot generation in an explicit script such as `pnpm --filter @lynx-js/ui-judge run update:react-fixture-snapshot`, not in the normal test path.
