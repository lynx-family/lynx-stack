...
Keep Rust Kitten-Lynx/Android automation, screenshot alignment/diffing, visual model evaluation, Android `judgeAndroidAgent` scoring, GEQI dimension prompts, 0-5 score normalization, report generation, and UI Judge package tests under `packages/genui/ui-judge/rust` with crate metadata in `packages/genui/ui-judge/Cargo.toml`. Do not add Vitest tests for `@lynx-js/ui-judge`; screenshot/API coverage should be Rust unit or integration tests and must not require browsers, emulators, or real model credentials; use the Rust mock-response environment hook when needed.
