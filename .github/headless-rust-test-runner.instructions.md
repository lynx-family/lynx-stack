...
On non-macOS platforms, keep `runs_on_current_thread` false while `set_global_ui_task_runner` is installing the callback, then publish the process owner only after the surrounding `OnceLock::get_or_init` has returned. Publish it in the outer success branch, not from captured state inside the initializer; native teardown depends on this ordering.
