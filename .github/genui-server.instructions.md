...
Integrate the subscription-compatible Doubao Search Custom web API as the optional server-side Mastra `web_search` tool, enabled only by `SEARCH_INFINITY_API_KEY`; use `SEARCH_INFINITY_REQUEST_TIMEOUT_MS` for its bounded timeout and never accept search credentials or endpoints from clients. Keep the `web` search type, request parameters, and result sizes server-owned, normalize `WebResults` to bounded text summaries and source metadata, omit search images, and redact upstream response bodies from failures. Prefer each result's `Summary`, with `Snippet` and bounded `Content` only as fallbacks. Store the search call budget and returned document URLs in the same per-request RequestContext used by image generation so the limit spans initial generation and repairs. Allow `openUrl` only for URLs supplied in user-role messages or returned by the current search scope, with untrusted links withheld during streaming and rejected during final validation. Explicitly disable web search for every Bench path to preserve deterministic scoring.
