...
Build `genui-server` as an ESM Node.js handler through `rslib.config.ts`. Export only a named `handler(request, response)` function from `src/index.ts`; its request and response types must cover both Node HTTP/1 and HTTP/2, and the request must accept the platform-provided `path`, `queryStringParameters`, and `jsonBody` fields. Do not add a listening server, startup command, or process-lifecycle ownership. Keep protocol handlers based on standard Web `Request` and `Response`, register them in `src/routes.ts`, and keep Node HTTP stream conversion in `src/node-handler.ts`. Filter HTTP/2 pseudo-headers before constructing a Web `Headers` object. Do not introduce framework-specific request or response types into protocol routes.
