...
Build `genui-server` as an ESM Node.js handler through `rslib.config.ts`. Export only the standard Node `handler(IncomingMessage, ServerResponse)` function from `src/index.ts`; importing the handler must not create a listening server or take ownership of the process lifecycle. Keep the optional standalone Node HTTP host isolated in `scripts/start.mjs`; `pnpm start` serves the existing `dist/index.js` artifact and assumes the repository has already been built. Keep protocol handlers based on standard Web `Request` and `Response`, register them in `src/routes.ts`, and keep Node HTTP stream conversion in `src/node-handler.ts`. Do not introduce framework-specific request or response types into protocol routes.
