...
Build `genui-server` as an ESM Node.js handler through `rslib.config.ts`. Export only the standard Node `handler(IncomingMessage, ServerResponse)` function from `src/index.ts`; do not add a listening server, startup command, or process-lifecycle ownership. Keep protocol handlers based on standard Web `Request` and `Response`, register them in `src/routes.ts`, and keep Node HTTP stream conversion in `src/node-handler.ts`. Do not introduce framework-specific request or response types into protocol routes.
