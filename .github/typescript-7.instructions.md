...
When migrating Lynx Stack to TypeScript 7, keep the root compiler on TS7 but isolate tools that still depend on the legacy TypeScript JavaScript API, such as TypeDoc, typescript-eslint, or rsbuild-plugin-arethetypeswrong, by giving those packages a private TypeScript 5 dependency through pnpm hooks instead of relaxing the workspace TypeScript peer globally.
