...
For API report tasks that read package build outputs, depend on the package's own `build` task rather than a root transit task such as `//#build`; otherwise clean CI jobs can run root type checking before workspace declaration outputs exist.
