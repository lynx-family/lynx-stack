...
On Linux, a Turbo `outputs` glob like `dist/**` does not descend into hidden dot-directories (macOS does), so artifacts under paths like `dist-producer/.rspeedy/**` are silently dropped from the cache tarball and later jobs replay an incomplete build. Name each hidden directory explicitly in `outputs` (for example `examples/react-debug-metadata` lists both `dist-*/**` and `dist-*/.rspeedy/**`) instead of disabling caching.
