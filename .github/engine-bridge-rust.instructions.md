...
Runtime-backed tests require a Lynx runtime locally and in CI. Package `build.rs` files should prepare the default macOS dylib under `target/lynx-engine-bridge-sdk` and inject `LYNX_SDK_DIR`; avoid separate downloader scripts unless they are needed outside Cargo. Missing runtime configuration should fail runtime integration tests instead of skipping them.
