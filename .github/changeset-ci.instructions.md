...
When rejecting private packages declared in changesets, inspect `changesets[].releases` from the `changeset status --output` JSON. Do not inspect the top-level `releases` array because the final release plan filters private packages when `privatePackages.version` is disabled.
