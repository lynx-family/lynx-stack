...
When adding a built-in A2UI catalog component that should be available to the GenUI server agent, regenerate the complete server catalog at `packages/genui/server/agent/catalog.json` and provide a `COMPONENT_SUMMARIES` entry in `packages/genui/server/agent/a2ui-catalog.ts`. The server prompt catalog should read components and functions from the complete catalog file instead of importing per-component manifests or function files separately.

The complete generated A2UI catalog contains both `components` and `functions`, so keep it outside the per-component catalog directory. For `packages/genui/a2ui`, per-component manifests live under `dist/catalog/<ComponentName>/catalog.json`, while the complete catalog lives at `dist/catalog.json`. Keep the legacy package export `./catalog/catalog.json` as a compatibility alias to the complete `dist/catalog.json` rather than generating a second full catalog under `dist/catalog/`.

When evolving `packages/genui/playground`, treat protocol-prefixed hashes such as `#/a2ui/...` and `#/openui/...` as the canonical routes, and preserve the current mainline tab names (`create`, `examples`, `components`) when adding protocol-aware routing. If you keep compatibility aliases for older or transitional paths such as `#/demos` or `#/chat`, parse them into the canonical route model instead of letting a rebase silently rename the mainline routes.
