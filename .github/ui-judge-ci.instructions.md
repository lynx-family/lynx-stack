...
Publish the UI Judge Linux AMD64 image to `ghcr.io/lynx-family/ui-judge` from the deploy workflow only after the reusable Rust test workflow succeeds. Grant the publishing job only `contents: read` and `packages: write`, authenticate with `GITHUB_TOKEN`, and pin Docker-maintained actions to immutable commit SHAs. Publish a `latest` tag only from the default branch; publish sanitized branch and full commit-SHA tags for every deploy-workflow branch.
