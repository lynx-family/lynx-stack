...
When documenting GenUI CLI usage, use `a2ui-cli` as the only user-facing command entry point. Explain both command families: `generate catalog` for TypeScript-derived catalog artifacts and `generate prompt` for A2UI system prompts. Treat `@lynx-js/a2ui-catalog-extractor` as an internal implementation detail used by `a2ui-cli generate catalog`, not as an external API or recommended binary, and remind readers that generated prompts and client catalogs must agree on component names and props.
