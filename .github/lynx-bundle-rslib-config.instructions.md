...
Keep External Bundle debug metadata generic: drive main-thread classification from entry layers and `mainThreadChunks`, stamp each asset with its actual `customSections` path, and pass encoding through the standard `LynxTemplatePlugin` hooks. Never infer TASM routing from downstream business filenames or compatibility entry-name lists.
