---
applyTo: "packages/lynx/kitten-lynx-rust/**"
---

Keep `kitten-lynx-rust` as a Rust-only headless testing crate built on `packages/lynx/engine-bridge/lynx`. Preserve API and CDP behavior parity with `packages/testing-library/kitten-lynx` where the headless host supports it, but use the software renderer frame as the screenshot source and the local desktop DebugRouter instead of ADB.
