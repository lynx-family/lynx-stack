...
In spread attribute adapters, filter framework metadata and handle class, ref, event, worklet, and function-valued props before applying ordinary attribute-name normalization. Keep the shared camelCase-to-dash attribute-name normalizer as a pure character conversion; callers own all special-property decisions. Feature-gate normalization with a compilation macro so disabled builds can eliminate the branch without changing the spread object shape.
