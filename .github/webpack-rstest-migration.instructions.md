...
For `packages/webpack/react-refresh-webpack-plugin`, keep the successful hot-test migration minimal: switch shared helpers to repo-local imports from `packages/webpack/test-tools/lib/*`, keep repo-local `update.js` fixture paths throughout the hot-case fixture flow, and preserve the package's existing hot-case behavior instead of rewriting the test creator pattern.
...
When a workspace bumps `@rspack/test-tools` only to carry an ESM-config compatibility patch, register the exact patched package version in `pnpm-workspace.yaml#patchedDependencies` and refresh `pnpm-lock.yaml` so package installs consistently apply `patches/@rspack__test-tools@<version>.patch`.
