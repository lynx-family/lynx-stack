---
applyTo: "Dockerfile,.dockerignore,.github/workflows/deploy-main.yml"
---
...
Do not copy host `node_modules`, Cargo targets, Turbo caches, or local environment files into the Docker build context. Publish GHCR images with branch and commit tags, update `latest` only from `main`, and scope `packages: write` to the publishing job. Pin Docker Actions to verified commit SHAs.
